...
class GetSearchNote {
  ...
  GetSearchNote({ required this.collection , required this.search});

  final db = FirebaseFirestore.instance;

  Widget get searchNote{
    return Padding(
      padding: const EdgeInsets.only(left: 10,right: 10),
      child: StreamBuilder<QuerySnapshot>(
        // stream: db.collection(collection).orderBy("title").startAt([search]).snapshots(),
        stream: db.collection(collection).where("title", isEqualTo: search).snapshots(),
